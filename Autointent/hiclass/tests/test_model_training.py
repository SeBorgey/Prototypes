...
SAMPLE_TRAIN_DATA_RAW = [
    {"text": "What is biology?", "labels": ["science", "biology"]},
    {"text": "Study of cells", "labels": ["science", "biology"]},
    {"text": "Tell me about Shakespeare", "labels": ["arts", "literature"]},
    {"text": "What is a sonnet?", "labels": ["arts", "literature"]},
    {"text": "What is physics?", "labels": ["science", "physics"]},
]
SAMPLE_TEST_DATA_RAW = [
    {"text": "Plant and animal cells", "labels": ["science", "biology"]},
    {"text": "Romeo and Juliet", "labels": ["arts", "literature"]},
]
...
def preprocessed_data():
    preprocessor = DatasetPreprocessor(
        SAMPLE_TRAIN_DATA_RAW, SAMPLE_TEST_DATA_RAW, min_train_freq=2
    )
...
def test_hiclass_trainer(preprocessed_data):
    ...
    mlb = MultiLabelBinarizer(classes=preprocessed_data.final_labels).fit(
        [set(s["labels"]) for s in train_f]
    )
...
def test_autointent_multiclass_trainer(preprocessed_data):
    ...
    trainer = AutoIntentMulticlassTrainer(
        embedder_config, val_size=0.5, random_state=42
    )

    leaf_to_id = {leaf: i for i, leaf in enumerate(preprocessed_data.common_leaves)}

    prepared_data = trainer.prepare(
        train_f,
        test_f,
        leaf_to_id=leaf_to_id,
        final_labels=preprocessed_data.final_labels,
    )
...
def test_autointent_multilabel_trainer(preprocessed_data):
    train_f, test_f = preprocessed_data.get_processed_data()
    embedder_config = EmbedderConfig(model_name=EMBEDDER_MODEL)
    trainer = AutoIntentMultilabelTrainer(
        embedder_config, val_size=0.5, random_state=42
    )
    ...
    prepared_data = trainer.prepare(
        train_f, test_f, mlb=mlb, final_labels=preprocessed_data.final_labels
    )
