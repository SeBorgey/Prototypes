...
def test_metrics_for_hiclass(shared_context):
    y_true_raw = np.array(
        [["science", "biology", ""], ["arts", "literature", ""]], dtype=object
    )

    y_pred = {
        "predictions": np.array(
            [["science", "biology", ""], ["arts", "biology", ""]], dtype=object
        ),
        "scores": np.array([[0.8, 0.7, 0.1, 0.1], [0.1, 0.6, 0.7, 0.2]]),
    }
...
def test_metrics_for_multilabel():
    ...
    y_pred = {
        ...
        "scores": np.array([[0.9, 0.8, 0.1, 0.2], [0.7, 0.3, 0.6, 0.1]]),
    }
