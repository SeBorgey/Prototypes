...
HICLASS_MODELS = {
    "hiclass LCPN (siblings)": LocalClassifierPerNode(binary_policy="siblings", n_jobs=-1),
...
#     try:
#         df_train = kagglehub.dataset_load(kagglehub.KaggleDatasetAdapter.PANDAS, dataset_handle, "DBPEDIA_train.csv")
#         df_test = kagglehub.dataset_load(kagglehub.KaggleDatasetAdapter.PANDAS, dataset_handle, "DBPEDIA_test.csv")
#         df = pd.concat([df_train, df_test], ignore_index=True)
#     except Exception as e:
#         print(f"Ошибка загрузки DBPedia: {e}")
#         print("Убедитесь, что у вас настроен kagglehub (pip install kagglehub; kagglehub auth login).")
...
#     mapping_path = hf_hub_download(repo_id=dataset_id, filename="label.json", repo_type="dataset")
#     with open(mapping_path, 'r') as f:
#         id_to_label = json.load(f)
#     
#     full_df = pd.concat([dataset_dict['train'].to_pandas(), dataset_dict['validation'].to_pandas(), dataset_dict['test'].to_pandas()])
#
...
def evaluate_autointent_multiclass(X_train_list, y_train, X_test_list, y_test, embedder_config):
    ...
    y_pred_paths = [leaf_to_path.get(leaf, [f"unknown_leaf_{leaf}"]) for leaf in y_pred_flat]
    
    metrics = calculate_hierarchical_metrics(y_test, y_pred_paths)
    metrics["model"] = "AutoIntent (LogReg Multiclass)"
    return metrics

def evaluate_autointent_multilabel(X_train_list, y_train, X_test_list, y_test, embedder_config):
...
def plot_results(df):
    for dataset_name in df['dataset'].unique():
        ...
        ax.set_title(f'Сравнение моделей на датасете: {dataset_name}', fontsize=18, pad=20)
...
def main():
    ...
    for name, loader in datasets_to_run.items():
        ...
        print("\nОценка AutoIntent (Multiclass)...")
        autointent_mc_results = evaluate_autointent_multiclass(X_train_list, y_train, X_test_list, y_test, EMBEDDER_CONFIG)
        ...
        autointent_ml_results = evaluate_autointent_multilabel(X_train_list, y_train, X_test_list, y_test, EMBEDDER_CONFIG)
